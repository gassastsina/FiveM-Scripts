...
AddEventHandler('es_admin:ban', function(id)
	for k,v in ipairs(GetPlayerIdentifiers(id))do
		banUser(v)
	end
	DropPlayer(id, GetConvar("es_admin_banreason", "Tu as été ban de ce serveur"))
	TriggerEvent('logs:write', "<@&464073491063373825> A été ban du serveur !", id)
end)
...
AddEventHandler('playerConnecting', function(user, set)
	for k,v in ipairs(GetPlayerIdentifiers(source))do
		if isBanned(v) then
			set(GetConvar("es_admin_banreason", "Tu es ban de ce serveur"))
			CancelEvent()
			break
		end
	end
...
AddEventHandler('es_admin:all', function(type)
	...
	TriggerEvent('es:getPlayerFromId', source, function(user)
		TriggerEvent('es:canGroupTarget', user.getGroup(), "admin", function(available)
			if available or user.getGroup() == "superadmin" or user.getGroup() == "_dev" then
	...
	if args[1] then
		if(GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				if(#reason == 0)then
					reason = "Kicked: Tu as été du du serveur"
				else
		...
		if(GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				if(target)then
					...
					TriggerClientEvent('chatMessage', player, "SYSTEM", {255, 0, 0}, "Tu as été téléporté par ^2" .. GetPlayerName(source))
					TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "téléporté à ^2" .. GetPlayerName(player) .. "")
				end
			end)
		else
			TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Incorrect player ID!")
