...
Citizen.CreateThread(function()
	local blip = AddBlipForCoord(Config.AgentImmo.x, Config.AgentImmo.y, Config.AgentImmo.z)
	SetBlipSprite (blip, 475)
	SetBlipDisplay(blip, 4)
	SetBlipScale  (blip, 1.0)
	SetBlipAsShortRange(blip, true)
	BeginTextCommandSetBlipName("STRING")
	AddTextComponentString("Agent Immobilier")
	...
	while true do
		...
		if Vdist(coords.x, coords.y, coords.z, Config.AgentImmo.x, Config.AgentImmo.y, Config.AgentImmo.z) < Config.MarkerSize.x then
			SetTextComponentFormat('STRING')
			AddTextComponentString("Appuyez sur ~INPUT_CONTEXT~ pour voir les propriétés")
			DisplayHelpTextFromStringLabel(0, 0, 1, -1)
			if IsControlJustPressed(0, 38) then
				if not seeBlips then
					TriggerEvent('esx:showNotification', 'Les propriétés sont visibles sur la carte')
					CreateBlips()
					seeBlips = true
				else
					for i=1, #Config.Properties, 1 do
						local property = Config.Properties[i]
						if GetBlipSprite(Blips[property.name]) ~= 357 then
							RemoveBlip(Blips[property.name])
						end
					end
					TriggerEvent('esx:showNotification', 'Toutes les propriétés ont disparues de votre carte')
					seeBlips = false
				end
			end
		end
...
function CreateBlips()
  ...
  for i=1, #Config.Properties, 1 do
    ...
    if property.entering ~= nil then
      ...
      SetBlipScale  (Blips[property.name], 1.2)
...
function OpenPropertyMenu(property)
  ...
  if PropertyIsOwned(property) then
    ...
    if not Config.EnablePlayerManagement then
      table.insert(elements, {label = _U('buy')..' pour <span style="color:green;">'..property.price..'$</span>', value = 'buy'})
      table.insert(elements, {label = _U('rent')..' pour <span style="color:green;">'..tostring(property.price/100)..'$</span>',   value = 'rent'})
    end
    ...
    function(data, menu)
      ...
      ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gateway_available_properties_actions',
        {
          title    = property.name,
          elements = {
            {label = _U('buy')..' pour <span style="color:green;">'..data.current.price..'$</span>', value = 'buy'},
            {label = _U('rent')..' pour <span style="color:green;">'..tostring(data.current.price/100)..'$</span>',   value = 'rent'},
...
function OpenRoomInventoryMenu(property, owner)
  ...
  ESX.TriggerServerCallback('esx_property:getPropertyInventory', function(inventory)
    ...
    for i=1, #inventory.weapons, 1 do
      ...
      table.insert(elements, {label = weapon.label, type = 'item_weapon', value = weapon.name})
    end
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if data.current.type == 'item_weapon' then
          ...
          TriggerServerEvent('esx_property:getItem', owner, data.current.type, data.current.value)
  ...
  ESX.TriggerServerCallback('esx_property:getPlayerInventory', function(inventory)
    ...
    for i=1, #weaponList, 1 do
      ...
      if HasPedGotWeapon(playerPed,  weaponHash,  false) and weaponList[i].name ~= 'WEAPON_UNARMED' then
        ...
        table.insert(elements, {label = weaponList[i].label, type = 'item_weapon', value = weaponList[i].name})
      end
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if data.current.type == 'item_weapon' then
          ...
          TriggerServerEvent('esx_property:putItem', owner, data.current.type, data.current.value)
