...
function OpenShopMenu(zone)
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			 ESX.UI.Menu.Open(
				'dialog', GetCurrentResourceName(), 'dialog_garageEdit_menu',
					{
						title = 'quantité '
				},
			    function(data2, menu2)
			    	local qty = tonumber(data2.value)
					TriggerServerEvent('esx_shop:buyItem', data.current.value, data.current.price,qty)
				end,
				function(data2, menu2)
					menu2.close()
				end
