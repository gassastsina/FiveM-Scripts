...
local function IsItem(zone)
	if zone == 'BlackItems' then
		return 'items'
	end
	return 'weapons'
end

function OpenWeaponsShopMenu(zone)
	local elements = {}
	...
	if zone == 'BlackItems' then
		table.insert(elements, {label = 'Changer de numéro de téléphone', value = 'change_num', price = 400})
	end

	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_weapons',
	{
		title  = _U('weapons'),
		elements = elements
	}, function(data, menu)
		if data.current.value ~= 'change_num' then
			TriggerServerEvent('esx_weashop:buyItem', data.current.value, data.current.price, zone, data.current.type, data.current.realLabel)
		else
			menu.close()
			TriggerServerEvent('esx_weashop:removeMoneyToChangeNum', data.current.price)
		end
	end, function(data, menu)
		menu.close()
...
AddEventHandler('informant:openIllegalSeller', function(shop)
	OpenWeaponsShopMenu(shop)
end)