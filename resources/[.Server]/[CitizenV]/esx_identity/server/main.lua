...
function setIdentity(identifier, data, callback)
	MySQL.Async.execute("UPDATE `users` SET `firstname`=@firstname, `lastname`=@lastname, `dateofbirth`=@dateofbirth, `sex`=@sex, `height`=@height, `numCard`=@numCard WHERE identifier=@identifier",
	{
		['@identifier']		= identifier,
		...
		['@sex']			= data.sex,
		['@height']			= data.height,
		['@numCard']		= 'C0001'..math.random(111, 999)
	}, function(done)
		if callback then
			callback(true)
		end
...
TriggerEvent('es:addGroupCommand', 'chardel', 'user', function(source, args, user)
	...
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Permission insuffisante' } })
end, {help = "Delete a registered character", params = {{name = "char", help = "the character id, ranges from 1-3"}}})



--FirstConnection
RegisterServerEvent('esx_identity:addMoneyAndItems')
AddEventHandler('esx_identity:addMoneyAndItems', function(bank, money, items, vehicle)
	local xPlayer = ESX.GetPlayerFromId(source)
	xPlayer.addMoney(money)
	xPlayer.addBank(bank)

	for i=1, #items, 1 do
		xPlayer.addInventoryItem(items[i].name, items[i].qty)
	end
	if vehicle ~= nil then
		TriggerEvent('esx_vehicleshop:setVehicleOwnedPlayerId', source, vehicle)
	end
end)