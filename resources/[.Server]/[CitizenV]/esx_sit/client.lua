...
----    Side : client         							 ----
----    Description : Sit on chairs or bench in the city ----
-------------------------------------------------------------
-------------------------------------------------------------
...
Citizen.CreateThread(function()
	...
	local stockCoords = nil
	while true do
		Wait(10)
		--local object, distance = ESX.Game.GetClosestObject(objects)
		if IsControlPressed(1, Keys["E"]) or (IsControlJustPressed(0, Keys['SPACE']) and sitting) then
			...
			for k,v in pairs(objects) do
				...
				table.insert(list, {object = obj, distance = GetDistanceBetweenCoords(GetEntityCoords(ped), GetEntityCoords(obj), true)})
			end
			...
			if distance < Config.MaxDistance and not sitting and DoesEntityExist(object) then
				headsUp('Appuyez sur ~INPUT_FRONTEND_ACCEPT~ pour vous asseoir.')
				...
				if IsControlJustPressed(0, Keys['ENTER']) or IsControlJustPressed(0, Keys['NENTER']) then
					sit(object)
					Wait(1800)
					stockCoords = GetEntityCoords(GetPlayerPed(-1), false)
				end
			end
			if sitting then
				headsUp('Appuyez sur ~INPUT_JUMP~ pour vous lever.')
				if IsControlJustPressed(0, Keys['SPACE']) or (stockCoords ~= nil and stockCoords ~= GetEntityCoords(GetPlayerPed(-1), false)) then
					ClearPedTasks(ped)
					sitting = false
...
function sit(object)
	...
	if isOccupied == false then
		...
		TaskStartScenarioAtPosition(ped, objinfo.scenario, objloc.x, objloc.y, objloc.z-objinfo.verticalOffset, GetEntityHeading(object)+180.0, -1, true, true)
		TriggerEvent('esx_foodtruck:getSittingStatus')
	end
end