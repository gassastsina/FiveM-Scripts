...
function AddLicense(target, type, cb)
	...
	if type ~= 'dmv' then
		addPointsInDB(xPlayer.identifier)
	end
end

function addPointsInDB(identifier)
	MySQL.Async.insert("UPDATE users SET licence_points=@licence_points WHERE identifier=@identifier", {
			--Condition
