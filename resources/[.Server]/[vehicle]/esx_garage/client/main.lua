...
AddEventHandler('esx_property:hasEnteredMarker', function(name, part, parking)
			...
			if healthPercent < Config.MinimumHealthPercent then
				...
				if GetPedInVehicleSeat(vehicle,  -1) == playerPed then
					...
					local spawnCoords  = {
		  			...
		  			TriggerServerEvent('tracker:removeVehicle', GetVehicleNumberPlateText(vehicle))
							...
							for j=1, #vehicles, 1 do
								...
								if i == vehicles[j].zone then
									...
									local spawn = function(j)
										...
										TriggerServerEvent('logs:write', 'Rentre un véhicule ('..vehicles[j].vehicle.model.plate..') dans son garage : ')
							...
							if i == vehicles[j].zone then
								...
								local spawn = function(j)
									...
									ESX.Game.SpawnLocalVehicle(vehicles[j].vehicle.model, {
										...
										ESX.Game.SetVehicleProperties(vehicle, vehicles[j].vehicle)
	  									TriggerEvent('esx_key:getVehiclesKey', vehicles[j].vehicle.plate)
									end)
	...
	if part == 'InteriorExitPoint' then
		...
		if IsPedInAnyVehicle(playerPed,  false) then
			...
			TriggerServerEvent('logs:write', 'Sort le véhicule ('..vehicleProps.plate..') de son garage ')
