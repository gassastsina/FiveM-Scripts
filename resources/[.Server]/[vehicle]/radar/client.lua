	...
	while true do
		...
		if(radarActivated == true) then
			...
			if(IsPedInAnyPoliceVehicle(GetPlayerPed(-1))) then
				...
				DrawMarker(1,x,y,z,0,0,0,0,0,0,8.001,8.0001,2.0001,0,155,255,200,0,0,0,0)
...
RegisterNetEvent("radar:open")
AddEventHandler("radar:open", function()
	menu = true
end)
...
function tick()
	...
	if(IsPedInAnyPoliceVehicle(GetPlayerPed(-1))) then
		if not IsPedInAnyHeli(GetPlayerPed(-1)) then
			if IsControlPressed(1, 202) then --Back
				menu = false
			end
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if(menu) then
			...
			TriggerEvent("radarGUI:Int", text[lang].menuDeplacementMultiplier, multiplier, 1, 20, function(cb)
				multiplier = cb
			end)
