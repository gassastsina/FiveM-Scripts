...
local function addEssenceToVehicle(result, amount, toPay, _source, index)

	if result[1].quantity - amount > 0 then
		ESX.GetPlayerFromId(_source).removeMoney(math.ceil(toPay))
		TriggerClientEvent("essence:hasBuying", _source, amount)
		TriggerEvent('SAP:removePetrolFromStation', index, amount)
	elseif result[1].quantity > 0 then
		TriggerClientEvent("showErrorNotif", _source, "Cette station n'a plus autant d'essence")
	else
		TriggerClientEvent("showErrorNotif", _source, "Cette station est vide")
	end
end
...
AddEventHandler("essence:buy", function(amount, index, e)
	...
	if(toPay > ESX.GetPlayerFromId(_source).getMoney()) then
		TriggerClientEvent("showErrorNotif", _source, "Vous n'avez pas assez d'argent")
	else
		if index < 26 or index > 35 then
			MySQL.Async.fetchAll('SELECT `quantity` FROM `gas_stations` WHERE stationNumber=@stationNumber', {
					['@stationNumber'] = index
				}, function(result)
					addEssenceToVehicle(result, amount, toPay, _source, index)
		 		end
		 	)
		else
			addEssenceToVehicle({{quantity = 500}}, amount, toPay, _source, index)
		end
	end
end)
