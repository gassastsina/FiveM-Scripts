...
Citizen.CreateThread(function()
	...
	while true do
		...
		if(IsPedInAnyVehicle(GetPlayerPed(-1), -1) and GetPedVehicleSeat(GetPlayerPed(-1)) == -1 and not isBlackListedModel()) then
			...
			if(vitesse > 1 and vitesse <20) then
				stade = 0.00002
				Wait(1500)
			...
			elseif(vitesse >=90 and vitesse <130) then
				stade = 0.00013
				Wait(1500)
			elseif(vitesse >=130 and vitesse <170) then
				stade = 0.00017
				Wait(1500)
			elseif(vitesse >=170 and vitesse <200) then
				stade = 0.00021
				Wait(1500)
			elseif(vitesse >=200 and vitesse <230) then
				stade = 0.00028
				Wait(1500)
			elseif(vitesse >= 230) then
				stade = 0.00035
				Wait(1500)
			elseif not IsVehicleEngineOn(GetVehiclePedIsIn(GetPlayerPed(-1))) then
				stade = 0
			elseif IsVehicleEngineOn(GetVehiclePedIsIn(GetPlayerPed(-1))) then
				stade = 0.00001
			end
	...
	if(IsPedInAnyVehicle(GetPlayerPed(-1), -1) and GetPedVehicleSeat(GetPlayerPed(-1)) == -1 and not isBlackListedModel()) then
		if showHUD then
			...
			if showText then
				...
				if round(percent,1) <= 15 then
					r, g, b = 200, 0, 0
				elseif round(percent,1) <= 30 then
					r, g, b = 180, 115, 0
				end
				DrawAdvancedText(text_x, text_y, 0.005, 0.0028, 0.4, round(percent,1).."%", r, g, b, 255, 0, 1)
			end
		end
	end
end
