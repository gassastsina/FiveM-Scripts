		...
		if DoesEntityExist(vehicle) then
			if IsVehicleDriveable(vehicle) then
				if GetEntityModel(vehicle) == GetHashKey("taxi") then
					...
					if driver then
						if GetDistanceBetweenCoords(Px, Py, Pz, x, y, z, true) <= 7.0 then
							...
							if GetGameTimer() < TaxiInfoTimer + 6000 then
								...
								if IsControlJustPressed(1, 23) then
									Citizen.CreateThread(function()
										while GetEntityModel(GetVehiclePedIsUsing(GetPlayerPed(-1))) ~= GetHashKey('TAXI') do Wait(100) end
	  									TriggerEvent('esx_key:getVehiclesKey', GetVehicleNumberPlateText(_vehicle))
										if DoesBlipExist(taxiBlip) then
											RemoveBlip(taxiBlip)
										end
									end)
...
AddEventHandler("fs_taxi:callTaxi", function()
	SpawnTaxi()
end)
	...
	while true do
		...
		if taxiService then
			if IsPedInVehicle(PlayerPedId(), data.vehicle, true) then
				...
				if GetDistanceBetweenCoords(pcoords.x, pcoords.y, pcoords.z, dx, dy, z, false) <= 100.0 then
					...
					if not reachedDest then
						...
						TriggerServerEvent("fs_taxi:payCab", tonumber(disttom/40))
						reachedDest = true
					end
				end
			else
