...
ESX.RegisterServerCallback('esx_mecanojob:checkEnoughBlowpipeItem', function(source, cb)
    if ESX.GetPlayerFromId(source).getInventoryItem('blowpipe').count > 0 then
        cb(true)
    else
        cb(false)
    end
...
RegisterServerEvent('esx_mecanojob:impoundVehicle')
AddEventHandler('esx_mecanojob:impoundVehicle', function(vehicleProperties)
	local _source = source
	MySQL.Async.execute('DELETE FROM user_parkings WHERE `plate`=@plate LIMIT 1', {
        ['@plate'] = vehicleProperties.plate
    })
	MySQL.Async.fetchAll('SELECT * FROM `owned_vehicles` WHERE 1', {}, 
		function(vehicles)
 			for i=1, #vehicles, 1 do
 				if json.decode(vehicles[i].vehicle).plate == vehicleProperties.plate then
					MySQL.Async.execute('INSERT INTO impound (identifier, vehicle) VALUES (@identifier, @vehicle)', {
					  ['@identifier']  = vehicles[i].owner,
					  ['@vehicle'] 	   = vehicleProperties
					},
					function(rowsChanged)
						if cb ~= nil then cb() end
					end)
 					TriggerEvent('logs:write', 'Vient de mettre en fourrière le véhicule ('..vehicleProperties.plate..') appartenant à "'..vehicles[i].owner..'"', _source)
					break
 				end
 			end
 		end
