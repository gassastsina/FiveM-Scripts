        ...
        for l,w in pairs(v.Zones) do
          if (w.Type == "vehdelete") --[[and (w.Spawner == zone.Spawner)]] then
            ...
            if IsPedInAnyVehicle(playerPed, 0) then
                ...
                if PlayerData.job.name == "slaughterer" and playerPed == GetPedInVehicleSeat(vehicle, -1) then
                    TriggerServerEvent('esx_society:putVehicleInGarage', 'slaughterer', ESX.Game.GetVehicleProperties(vehicle))
                    DeleteVehicle(vehicle)
                    SetEntityAsNoLongerNeeded(vehicle)
                else
                    --for i=1, #myPlate, 1 do
                      if --[[(myPlate[i] == plate) and]] (playerPed == GetPedInVehicleSeat(vehicle, -1)) then
    ...
    ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(garageVehicles)
      ...
      ESX.UI.Menu.Open(
        ...
        function(data, menu)
          ...
          ESX.Game.SpawnVehicle(vehicleProps.model, spawnPoint.Pos, spawnPoint.Heading, function(vehicle)
        ...
        if isInMarker and not menuIsShowed then
          ...
          elseif zone.Type == "vehdelete" and (onDuty) then
            ...
            if IsPedInAnyVehicle(playerPed, 0) then
              ...
              if PlayerData.job.name == 'slaughterer' then
                hintToDisplay = "Appuyez sur ~INPUT_CONTEXT~ pour ranger le véhicule"
              end
            else
