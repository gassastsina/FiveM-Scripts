...
TriggerEvent('es:addGroupCommand', 'playanim', 'admin', function(source, args, user)
  TriggerClientEvent('esx:playAnim', args[1], args[2], args[3])
end, function(source, args, user)
  TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Insufficient Permissions.")
...
TriggerEvent('es:addGroupCommand', 'givemoney', 'admin', function(source, args, user)
  ...
  if amount ~= nil then
    ...
    TriggerEvent('logs:write', "Vient de give "..amount.."$ à "..GetPlayerName(args[1]).." ("..xPlayer.identifier..")", _source)
  else
	  ...
	  if amount ~= nil then
	    if xPlayer.getAccount(account) ~= nil then
	      xPlayer.addAccountMoney(account, amount)
	    	TriggerEvent('logs:write', "Vient de give "..amount.."$ à "..GetPlayerName(args[1]).." ("..xPlayer.identifier..") sur le compte "..tostring(account), _source)
	    else
