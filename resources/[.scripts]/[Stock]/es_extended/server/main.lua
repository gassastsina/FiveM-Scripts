...
Citizen.CreateThread(function()
	Wait(20000)
	TriggerEvent('esx:GetFoodItemsList', Config.FoodItems)
end)
...
AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount)
  ...
  if type == 'item_standard' then
    ...
    if itemCount > 0 and sourceItem.count >= itemCount then
      ...
      if (getPlayerWeight(targetXPlayer, itemName) + targetItem.limit*itemCount) > 10000 then
  ...
  elseif type == 'item_weapon' then
    ...
    for i=1, #Config.Weapons, 1 do
      if Config.Weapons[i].name == itemName then
	    ...
	    TriggerEvent('weaponsAccessories:giveWeapon', sourceXPlayer.loadout, itemName, target)
	    sourceXPlayer.removeWeapon(itemName)
	    targetXPlayer.addWeapon(itemName, itemCount)

	    TriggerClientEvent('esx:showNotification', _source, _U('yougave') .. ' ~g~' .. Config.Weapons[i].label .. _U('to')   .. targetXPlayer.name)
	    TriggerClientEvent('esx:showNotification', target,  _U('youreceived')  .. ' ~g~' .. Config.Weapons[i].label .. _U('by') .. sourceXPlayer.name)
        break
      end
    end
  end
...
local function IsDrugItem(item)
	for i=1, #Config.DrugItems, 1 do
		if Config.DrugItems[i] == item then
			return Config.RemoveWeaponItemDelay
		end
	end
	return Config.RemoveInventoryItemDelay
end
  ...
  if type == 'item_standard' then
    ...
    if itemCount == nil or itemCount <= 0 then
      ...
      if itemCount > foundItem.count then
        ...
        TriggerClientEvent('esx:showNotification', _source, "~r~Suppression~s~ dans "..tostring(IsDrugItem(itemName)/1000).." secondes")

        SetTimeout(IsDrugItem(itemName), function()
  ...
  elseif type == 'item_account' then
    ...
    if itemCount == nil or itemCount <= 0 then
      ...
      if itemCount > xPlayer.getAccount(itemName).money then
        ...
        TriggerClientEvent('esx:showNotification', _source, "~r~Suppression~s~ dans 30 secondes")

        SetTimeout(Config.RemoveWeaponItemDelay, function()
  ...
  elseif type == 'item_weapon' then
    ...
    SetTimeout(Config.RemoveWeaponItemDelay, function()
      ...
      if Config.EnableWeaponPickup then
        TriggerEvent('weaponsAccessories:pickupLoadout', weaponName, xPlayer)
...
AddEventHandler('esx:onPickup', function(id)
  ...
  if pickup.type == 'item_standard' then
    ...
    local canTake   = (Config.MaxInventoryWeight - item.count > 0) and (Config.MaxInventoryWeight - item.count) or 0
    local total     = pickup.count < canTake and pickup.count or canTake
    local remaining = pickup.count - total

    if getPlayerWeight(xPlayer, pickup.name) + xPlayer.getInventoryItem(pickup.name).limit*pickup.count <= 10000 then
