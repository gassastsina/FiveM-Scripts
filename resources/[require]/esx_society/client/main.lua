...
function OpenBossMenu(society, close, options)
  ...
  if options.chest then
    table.insert(elements, {label = 'Coffre', value = 'items_chest'})
  end
  ...
  if options.blackmoney then
    table.insert(elements, {label = "Argent sale", value = 'blackmoney'})
  end
  ...
  ESX.TriggerServerCallback('esx_society:getSocietyMoney', function(money)
      title = 'Patron <span style="color:green;">'..tostring(money)..'$</span>'
  end, society)
  while title == nil do
  	Wait(100)
  end
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      if data.current.value == 'items_chest' then
      	OpenInventoryMenu(society)
      end
      ...
      elseif data.current.value == 'blackmoney' then
	    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'choice_inventory_menu',
	        {
	        title    = 'Coffre',
	        elements = {
		        {label = "Déposer de l'argent sale", value = 'put'},
		        {label = "Récupérer de l'argent sale", value = 'get'}
		    },
	        },
	        function(data, menu)
	            if data.current.value == 'put' then
	                PutBlackMoney(society, close, options)
	            elseif data.current.value == 'get' then
	                GetBlackMoney(society, close, options)
	            end
	        end,
	        function(data, menu)
	            menu.close()
...
----------------BLACK MONEY-----------------
function GetBlackMoney(society, close, options)
    ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'withdraw_society_blackmoney_amount_' .. society, {
        title = "Montant de l'argent sale"
      },
      function(data, menu)
        ...
        if amount == nil then
          ...
          menu.close()
          TriggerServerEvent('esx_society:GetBlackMoney', society, amount)
          Wait(100)
          OpenBossMenu(society, close, options)
        end
...
function PutBlackMoney(society, close, options)
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'deposit_blackmoney_amount_' .. society, {
	    title = "Montant de l'argent sale"
	  },
	  function(data, menu)
	    ...
	    if amount == nil then
	      ...
	      menu.close()
	      TriggerServerEvent('esx_society:PutBlackMoney', society, amount)
	      Wait(100)
	      OpenBossMenu(society, close, options)
	    end
...
----------------DÉBUT COFFRE----------------
function OpenInventoryMenu(society)
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
            ...
            if data.current.value == 'put' then
                PutInventory(society)
            elseif data.current.value == 'get' then
                GetInventory(society)
            end
        end,
        function(data, menu)
            menu.close()
...
function PutInventory(society)

    ESX.TriggerServerCallback('esx_society:getPlayerInventory', function(items)
            ...
            ESX.UI.Menu.Open(
              ...
              function(data2, menu2)
                ...
                if count == nil then
                  ...
                  menu2.close()
                  TriggerServerEvent('esx_society:putStockItem', itemName, count, society)
                  PutInventory(society)
                end
