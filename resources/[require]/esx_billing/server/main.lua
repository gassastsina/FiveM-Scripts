			...
			if xTarget ~= nil then
				MySQL.Async.execute(
					...
					function(rowsChanged)
						...
						TriggerEvent('logs:write', "A reçu la facture "..label.." de "..amount.."$", xTarget.source)
					end
				)
			end
				...
				if xTarget ~= nil then
					...
					if xPlayer.getMoney() >= amount then
						...
						MySQL.Async.execute('DELETE from billing WHERE id = @id',
						{
							...
							TriggerEvent('logs:write', "A payé la facture "..result[1].label.." de "..amount.."$ de "..GetPlayerName(xTarget.source), xPlayer.source)
				...
				TriggerEvent('esx_addonaccount:getSharedAccount', target, function(account)
					...
					if xPlayer.getMoney() >= amount then
						...
						MySQL.Async.execute('DELETE from billing WHERE id = @id',
						{
							...
							TriggerClientEvent('esx:showNotification', xPlayer.source, _U('paid_invoice', amount))
							TriggerEvent('logs:write', "A payé la facture "..result[1].label.." de "..amount.."$", xPlayer.source)
							if xTarget ~= nil then
								TriggerClientEvent('esx:showNotification', xTarget.source, _U('received_payment', amount))
							end
